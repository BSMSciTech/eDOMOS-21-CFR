...
window.forceWebSocket = function() {
    ...
    if (!socket || !socket.connected) {
        ...
        if (typeof io !== 'undefined') {
            socket = io('/events', {
                transports: ['polling'],  // Use polling only to avoid WebSocket WSGI errors
                reconnection: true,
                forceNew: true
            });
        
        // Export to window for access from event_log.html
        window.socket = socket;
        console.log("✅ Socket exported to window.socket");
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof io !== 'undefined') {
        ...
        socket.on('new_event', function(data) {
            ...
            if (data.event) {
                // Notifications removed: No browser or toast popups for WebSocket events
                // Only update UI elements silently
            }
...
function processEvent(data) {
    ...
    try {
        ...
        if (data.statistics || data.door_status || data.alarm_status) {
            console.log('🎯 CALLING updateDashboardStats with:', {
                hasStatistics: !!data.statistics,
                hasDoorStatus: !!data.door_status,
                hasAlarmStatus: !!data.alarm_status,
                totalEvents: data.statistics?.total_events
            });
...
function updateEventLogTable(event) {
    ...
    if (tbody.querySelector(`tr[data-event-id="${event.id}"]`)) {
        console.log('⚠️ Row already exists for event', event.id, '- skipping duplicate');
        return;
    }
    ...
    if (event.event_type === 'door_open') {
        badge = '<span class="badge bg-info"><i class="fas fa-door-open me-1"></i>Door Open</span>';
    } else if (event.event_type === 'door_close') {
        badge = '<span class="badge bg-success"><i class="fas fa-door-closed me-1"></i>Door Close</span>';
    } else if (event.event_type === 'alarm_triggered') {
        badge = '<span class="badge bg-danger"><i class="fas fa-exclamation-triangle me-1"></i>Alarm Triggered</span>';
    } else {
        badge = `<span class="badge bg-secondary">${event.event_type}</span>`;
    }
    
    // Create new row with proper styling to match server-rendered events
    ...
    console.log('✅ Row created for event', event.id, 'with spinner');
    ...
    pollForEventImage(event.id, newRow);
...
// Poll for event image (check every 1 second, max 10 attempts)
function pollForEventImage(eventId, row) {
    let pollCount = 0;
    const maxPolls = 10; // Increased from 5 to 10 seconds
    
    console.log(`🔄 Starting image polling for event ${eventId}`);
    
    // Start polling after a short delay to allow image capture to complete
    setTimeout(() => {
        const pollInterval = setInterval(() => {
            pollCount++;
            console.log(`🔍 Polling attempt ${pollCount}/${maxPolls} for event ${eventId} image...`);
            
            fetch(`/api/events/${eventId}`)
                .then(response => {
                    console.log(`📡 API response status for event ${eventId}:`, response.status, response.statusText);
                    if (!response.ok) {
                ...
                .then(updatedEvent => {
                    if (!updatedEvent) {
                        console.log(`⚠️ No data returned for event ${eventId}`);
                        return;
                    }
                    console.log(`📦 Updated event data for ${eventId}:`, updatedEvent);
                    
                    if (updatedEvent.image_path) {
                        console.log(`✅ Image found! Updating cell for event ${eventId}:`, updatedEvent.image_path);
                        
                        // Update the image cell
                        const imageCell = row.querySelector('.event-image-cell');
                        if (imageCell) {
                            const timestamp = new Date().getTime();
                            const eventType = updatedEvent.event_type || 'unknown';
                            const eventTimestamp = updatedEvent.timestamp || 'N/A';
                            const imageHash = updatedEvent.image_hash || 'N/A';
                            
                            imageCell.innerHTML = `
                                <img src="${updatedEvent.image_path}?v=${timestamp}" 
                                     alt="Event ${eventId}" 
                                     class="event-thumbnail animate__animated animate__fadeIn"
                                     onclick="openLightbox('${updatedEvent.image_path}', '${eventType}', '${eventTimestamp}', '${imageHash}')"
                                     style="cursor: pointer; width: 80px; height: 60px; object-fit: cover; border-radius: 4px;">
                            `;
                            console.log(`✅ Image cell HTML updated successfully!`);
                        }
                        
                        // Stop polling
                        clearInterval(pollInterval);
                        console.log(`🎉 Polling complete for event ${eventId}`);
                    } else {
                        console.log(`⏳ Image not yet available for event ${eventId}, will retry...`);
                    }
                })
                .catch(error => {
                    console.error(`❌ Error polling for event ${eventId}:`, error);
                });
            
            // Stop polling after max attempts
            if (pollCount >= maxPolls) {
                clearInterval(pollInterval);
                console.log(`⏹️ Stopped polling for event ${eventId} after ${maxPolls} attempts`);
                
                // Show "No Image" if still no image after max attempts
                const imageCell = row.querySelector('.event-image-cell');
                if (imageCell && imageCell.querySelector('.fa-spinner')) {
                    imageCell.innerHTML = '<span class="text-muted small">No Image</span>';
                }
            }
        }, 1000); // Poll every 1 second
    }, 500); // Wait 500ms before starting to poll (allows image capture to begin)
}
...
function updateDashboardStats(data) {
    ...
    if (data.statistics) {
        ...
        console.log('  ├─ Total Events: ' + data.statistics.total_events);
        console.log('  ├─ Door Open Events: ' + data.statistics.door_open_events);
        console.log('  ├─ Door Close Events: ' + data.statistics.door_close_events);
        console.log('  └─ Alarm Events: ' + data.statistics.alarm_events);
        ...
        console.log('  ├─ door-open-events found: ' + !!doorOpenEvents);
        console.log('  ├─ door-close-events found: ' + !!doorCloseEvents);
        console.log('  └─ alarm-events found: ' + !!alarmEvents);
        
        // Update statistics with animation
        if (totalEvents) {
            const oldValue = parseInt(totalEvents.textContent) || 0;
            console.log('🔄 UPDATING TOTAL EVENTS: ' + oldValue + ' → ' + data.statistics.total_events);
        ...
        console.log('✅ Statistics update complete:', data.statistics);
    } else {
        console.warn('⚠️ No statistics found in WebSocket data');
    }
}
...
function showEventNotification(event, source = 'unknown') {
    // Popups removed: do nothing for event notifications
    // All event popups (door open/close, alarm) are now suppressed
}
...
function initializeWebSocket() {
    ...
    if (typeof io !== 'undefined') {
        ...
        socket.on('connect', function() {
            ...
            if (eventQueue.length > 0) {
                console.log('Processing', eventQueue.length, 'queued events');
                eventQueue.forEach(data => processEvent(data));
                eventQueue = [];
            }
...
if (document.readyState === 'loading') {
    ...
    console.log('🚀 DOM ALREADY LOADED - INITIALIZING WEBSOCKET IMMEDIATELY');
    initializeWebSocket();
}

// Debug: Check if Socket.IO is available
console.log('🔍 DEBUGGING SOCKET.IO AVAILABILITY:');
console.log('  ├─ typeof io:', typeof io);
console.log('  ├─ io object:', typeof io !== 'undefined' ? 'AVAILABLE' : 'NOT AVAILABLE');
console.log('  ├─ Socket.IO version:', typeof io !== 'undefined' && io.version ? io.version : 'UNKNOWN');
